...
class HTMLFrameGenerator:
    ...
    def _check_linux_dependencies(self):
        ...
        try:
            ...
            result = subprocess.run(["fc-list"], capture_output=True, timeout=2)
    ...
    def get_media_size(self) -> tuple[int, int]:
        ...
        logger.warning(
            f"No media size meta tags found in template {self.template_path}, using fallback 1024x1024"
        )
        return 1024, 1024
    ...
    def parse_template_parameters(self) -> Dict[str, Dict[str, Any]]:
        ...
        for match in re.finditer(PARAM_PATTERN, self.template):
            ...
            if param_type not in {"text", "number", "color", "bool"}:
                logger.warning(
                    f"Unknown parameter type '{param_type}' for '{param_name}', defaulting to 'text'"
                )
                param_type = "text"
        ...
        if params:
            logger.debug(
                f"Parsed {len(params)} custom parameter(s) from template: {list(params.keys())}"
            )
    ...
    async def generate_frame(
        ...
        for attempt in range(2):
            try:
                ...
                if attempt == 0 and "handler is closed" in str(e):
                    logger.warning("Browser transport closed, reinitializing...")
                    HTMLFrameGenerator._browser = None
                    continue
                raise
        if page is None:
            raise RuntimeError("Failed to create browser page after retry")
