...
class VideoLipSyncPipelineUI(PipelineUI):
    ...
    def _render_lipsync_config(self) -> dict:
        with st.container(border=True):
            ...
            if not workflow_options:
                ...
                workflow_key = "runninghub/lip_sync_video_InfiniteTalk.json"
            else:
                default_idx = next(
                    (i for i, wf in enumerate(lipsync_workflows)
                     if wf["key"] == "runninghub/lip_sync_video_InfiniteTalk.json"), 0
                )
                ...
                try:
                    async def generate_lipsync_video():
                        ...
                        lip_synced = await self._run_lipsync_workflow(
                            ...
                            params.get("workflow_key", "runninghub/lip_sync_video_InfiniteTalk.json"),
                            task_dir,
                        )
    ...
    async def _run_lipsync_workflow(
        ...
        with open(f"workflows/{workflow_key}") as f:
